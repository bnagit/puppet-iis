...
define iis::manage_app_pool (
  ...
  if $apppoolrecycleschedule != undef {
    if (!empty($apppoolrecycleschedule)) {
      ...
      validate_re($restarttimesstring, '^\d{2}:\d{2}:\d{2}$|^\b\d{2}:\d{2}:\d{2}(?:,\b\d{2}:\d{2}:\d{2}\b)*$',
      "${restarttimesstring} bad - time format hh:mm:ss in array")
      ...
      $processscheduledtimes = true
    } # caller specified empty array - they want to clear scheduled recycles
  } else {
